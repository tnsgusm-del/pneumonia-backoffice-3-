...
| ID | 내용 |
| --- | --- |
| NFR-PTNT-001 | 모든 환자/진료 기록 API는 인증된 사용자(STAFF, ADMIN)만 접근 가능하다. |
| NFR-PTNT-002 | PENDING 권한 사용자는 모든 환자/진료 기록 API에 접근할 수 없다. |
| NFR-PTNT-003 | 모든 환자/진료 기록 API는 최대 3초 이내에 응답해야 한다. |
...
| --- | --- |
| API 이름 | 환자 등록 API |
| 설명 | 사내 의료인 역할을 가진 유저가 새로운 환자를 등록하는 API |
...
| name | string | Y | 환자 이름 |
| age | integer | Y | 환자 나이 |
| gender | string | Y | 환자 성별 (male / female) |
...
| name | string | 환자 이름 |
| age | integer | 환자 나이 |
| gender | string | 환자 성별 (male / female) |
...
| --- | --- |
| API 이름 | 환자 목록 조회 API |
| 설명 | 로그인된 사용자가 전체 환자 목록을 조회하는 API. 이름, 성별, 나이 범위로 필터링 가능 |
| 엔드포인트(Endpoint) | `/api/v1/patients` |
| 메서드(Method) | `GET` |
| 인증 필요 여부 | Y |
...
| gender | string | N | 성별 필터 (male / female) |
...
| --- | --- |
| API 이름 | 환자 삭제 API |
| 설명 | 특정 환자 정보 및 관련된 모든 진료 기록을 삭제하는 API |
| 엔드포인트(Endpoint) | `/api/v1/patients/{patient_id}` |
| 메서드(Method) | `DELETE` |
| 인증 필요 여부 | Y |
...
### 4. 비고
- 환자 삭제 시 해당 환자의 모든 진료 기록, X-Ray 이미지, AI 분석 결과가 연쇄 삭제됨 (CASCADE)

---

## REQ-MDR-001. 진료 기록 등록 API

### 1. API 개요
| 항목 | 내용 |
| --- | --- |
| API 이름 | 진료 기록 등록 API |
| 설명 | 사내 의료인 역할을 가진 유저가 특정 환자의 진료 기록을 등록하는 API. X-Ray 이미지 파일 포함 |
...
| 파라미터명 | 타입 | 필수 ( Y / N ) | 설명 |
| --- | --- | --- | --- |
| patient_id | integer | Y | 진료 기록을 등록할 환자의 ID |
| chart_number | string | Y | 환자 진료 차트 번호 (중복 불가) |
| symptoms | string | Y | 환자 증상 기록 |
| xray_image | file | Y | X-Ray 이미지 파일 (이미지 형식만 허용) |
...
- 진료 기록 등록은 STAFF, ADMIN 권한을 가진 유저만 가능

---

## REQ-MDR-002. 환자별 진료 기록 목록 조회 API

### 1. API 개요
| 항목 | 내용 |
| --- | --- |
| API 이름 | 환자별 진료 기록 목록 조회 API |
| 설명 | 특정 환자의 전체 진료 기록 목록을 조회하는 API |
...
### 4. 비고
- 진료 기록이 없을 경우 빈 배열 [] 반환

---

## REQ-MDR-003. 진료 기록 상세 조회 API

### 1. API 개요
| 항목 | 내용 |
| --- | --- |
| API 이름 | 진료 기록 상세 조회 API |
| 설명 | 특정 진료 기록의 상세 내용을 조회하는 API |
...
- 404 Not Found
      ...
      {
        "detail": "해당 진료 기록을 찾을 수 없습니다."
      }

| 필드명 | 타입 | 설명 |
| --- | --- | --- |
| detail | string | - record_not_found: 해당 진료 기록이 존재하지 않는 경우 |
